...
from linkedin_job.items import linkedin_jobItem
...
class linkedin_job(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        job_item = linkedin_jobItem()
        jobs = response.css('li')

        job_num = len(jobs)

        for job in jobs:
            job_item['job_title'] = job.css('a strong::text').get(default='not-found').strip()
            job_item['job_url'] = job.css('a::attr(href))').get(default='not-found').strip()
            job_item['job_location'] = job.css('.job-card-container__metadata-item::text').get(default='not-found').strip()
            job_item['job_company'] = job.css('span.job-card-container__primary-description::text').get(default='not-found').strip()
